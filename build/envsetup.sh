...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.colt.device | grep -q "$COLT_BUILD"); then
            ...
            echo "The connected device does not appear to be $COLT_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/Colt-Enigma/$PROJECT
...
function dopush()
{
    ...
    if (adb shell getprop ro.colt.device | grep -q "$COLT_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/colt/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $COLT_FIXUP_COMMON_OUT ]; then
